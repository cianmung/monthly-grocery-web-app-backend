...
const MonthYearFilter = () => {
  ...
  const years = paymentDetails.map(e => moment(new Date(e.date)).format("YYYY"));
  const months = paymentDetails.filter((each) => moment(new Date(each.date)).format("YYYY") === selectedYear).map(e => moment(new Date(e.date)).format("MMMM"));
  ...
  const setFilteredPaymentDetails = useStoreActions(actions => actions.setFilteredPaymentDetails);
  ...
  useEffect(() => {
    ...
    const selectedQuery = (paymentDetails.filter(each => moment(new Date(each.date)).format("MMMM") === selectedMonth && moment(new Date(each.date)).format("YYYY") === selectedYear));
